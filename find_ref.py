...
for which_class in ['Treat','Check']:
    ...
    co_score.append(np.dot(ref.flatten(), mask5.flatten()))
...
im3.paste(image_overlayed_F, (0, hh + 2 * space_height + 2 * HEIGHT - 150))
im3.paste(image_overlayed_C, (WIDTH + 10, hh + 2 * space_height + 2 * HEIGHT - 150))
im3.paste(image_overlayed_M, (2 * WIDTH + 20, hh + 2 * space_height + 2 * HEIGHT - 150))

draw = ImageDraw.Draw(im3)
font = ImageFont.truetype("arial.ttf", 70)

draw.text((1 / 2 * WIDTH, hh + 0 + HEIGHT-20), 'Nicolas : '+str(round(ni_score[0],2))+' , '+str(round(ni_score[1],2)), fill=(0, 0, 0), font=font)
draw.text((3 / 2 * WIDTH + 10, hh + 0 + HEIGHT-20), 'Jean : '+str(round(je_score[0],2))+' , '+str(round(je_score[1],2)), fill=(0, 0, 0), font=font)
draw.text((5 / 2 * WIDTH + 20, hh + 0 + HEIGHT-20), 'Guiseppe : '+str(round(gi_score[0],2))+' , '+str(round(gi_score[1],2)), fill=(0, 0, 0), font=font)
draw.text((1 / 2 * WIDTH, hh + 0 + 2 * HEIGHT + 70), 'Filippo : '+str(round(fi_score[0],2))+' , '+str(round(fi_score[1],2)), fill=(0, 0, 0), font=font)
draw.text((3 / 2 * WIDTH + 10, hh + 0 + 2 * HEIGHT + 70), 'Consensus : '+str(round(co_score[0],2))+' , '+str(round(co_score[1],2)), fill=(0, 0, 0), font=font)
draw.text((5 / 2 * WIDTH + 10, hh + 0 + 2 * HEIGHT + 70), 'Machine : '+str(round(ma_score[0],2))+' , '+str(round(ma_score[1],2)), fill=(0, 0, 0), font=font)
