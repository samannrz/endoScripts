...
def compute_NSD(mask_gt, mask_pred, tau,exterior_tolerance):

  # Dilate the masks to include exterior tolerance outside the mask boundaries
  dilated_mask_gt = binary_dilation(mask_gt, iterations=exterior_tolerance)
  dilated_mask_pred = binary_dilation(mask_pred, iterations=exterior_tolerance)

  # Compute distance transform for dilated masks
  dist_gt = distance_transform_edt(dilated_mask_gt == 0)  # Set background to 0
  dist_pred = distance_transform_edt(dilated_mask_pred == 0)  # Set background to 0

  # print(dist_gt.shape[0])
  # for i in range(dist_gt.shape[0]):
  #   for j in range(dist_gt.shape[1]):
  #     if dist_gt[i][j] < 0:
  #       print(dist_gt[i][j])

  # Define interior and exterior border regions for ground truth mask
  border_gt_interior = (dist_gt <= (tau+exterior_tolerance+tau)) & (dist_gt >= (exterior_tolerance))
  border_gt_exterior = (dist_gt <= (exterior_tolerance-tau)) & ~border_gt_interior

  # Define interior and exterior border regions for predicted mask
  border_pred_interior = (dist_pred <= (tau+exterior_tolerance+tau)) & (dist_pred >= (exterior_tolerance))
  border_pred_exterior = (dist_pred <= (exterior_tolerance-tau)) & ~border_pred_interior

  # Compute total border areas for ground truth and predicted masks
  total_border_area_gt = np.sum(border_gt_interior) + np.sum(border_gt_exterior)
  total_border_area_pred = np.sum(border_pred_interior) + np.sum(border_pred_exterior)

  # Compute overlap between border regions of ground truth and predicted masks
  overlap = (np.sum(border_gt_interior & border_pred_interior) +
             np.sum(border_gt_exterior & border_pred_exterior))

  # Compute the NSD
  nsd =(2.0 * overlap) / (total_border_area_gt + total_border_area_pred)
