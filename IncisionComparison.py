...
for j in range(math.ceil(lenimg / batch_size)):
    ...
    for i in range(j * batch_size, (j + 1) * batch_size):
        ...
        try:
            ...
            H2N = round((np.count_nonzero(maskH_2_array & maskH_N_array) / np.count_nonzero(maskH_2_array | maskH_N_array)) * 100, 2)
        except ZeroDivisionError:
            H2N = 1
        try:
            # score of Hard zones with ANNOT.1 & Nicolas
            H1N = round((np.count_nonzero(maskH_1_array & maskH_N_array) / np.count_nonzero(maskH_1_array | maskH_N_array)) * 100, 2)
        except ZeroDivisionError:
            H1N = 1
        try:
            # score of Hard zones with ANNOT.2 & JL
            H2J = round((np.count_nonzero(maskH_2_array & maskH_J_array) / np.count_nonzero(maskH_2_array | maskH_J_array)) * 100, 2)
        except ZeroDivisionError:
            H2J = 1
        try:
            # score of Hard zones with ANNOT.1 & JL
            H1J = round((np.count_nonzero(maskH_1_array & maskH_J_array) / np.count_nonzero(maskH_1_array | maskH_J_array)) * 100, 2)
        except ZeroDivisionError:
            H1J = 1
        try:
            # score of Check zones with ANNOT.1 & Nicolas
            S1N = round((np.count_nonzero(maskS_1_array & maskS_N_array) / np.count_nonzero(maskS_1_array | maskS_N_array)) * 100, 2)
        except ZeroDivisionError:
            S1N = 1
        try:
            # score of Check zones with ANNOT.2 & Nicolas
            S2N = round((np.count_nonzero(maskS_2_array & maskS_N_array) / np.count_nonzero(maskS_2_array | maskS_N_array)) * 100, 2)
        except ZeroDivisionError:
            S2N = 1
        try:
